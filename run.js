...
// -----------------------------------------------------------------------------
tasks.set('clean', () => del(['static/dist/*', '!static/dist/.git'], { dot: true }));
...
tasks.set('html', () => {
  ...
  const assets = JSON.parse(fs.readFileSync('./static/dist/assets.json', 'utf8'));
  const template = fs.readFileSync('./static/index.ejs', 'utf8');
  const render = ejs.compile(template, { filename: './static/index.ejs' });
  const output = render({ debug: webpackConfig.debug, bundle: assets.main.js, css: assets.main.css, config });
  fs.writeFileSync('./static/index.html', output, 'utf8');
});
